...
@asynccontextmanager
async def server_lifespan(server: Server, init_options: Optional[Dict[str, Any]] = None) -> AsyncIterator[AppContext]:
    """Initialize and clean up application resources."""
    ctx = init_options or {} # Directly use init_options
...
@app.read_resource()
async def read_resource(uri: AnyUrl) -> str: # Changed return type to str
    """Read a resource."""
    ctx = app.request_context.lifespan_context
    ...
    if str(uri).startswith("git://"):
        ...
        try:
            ...
            if stderr:
                return f"Error: {stderr}" # Return string directly
            ...
            return f"Error: {str(e)}" # Return string directly
    
    return f"Resource not found: {uri}" # Return string directly
